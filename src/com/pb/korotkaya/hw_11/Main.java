package com.pb.korotkaya.hw_11;
...
public class Main {
    ...
    private static void contactNew() {
        ...
        try {
            contacts.add( new Contact(name, phone, address, dateOfBirth, LocalDateTime.now()));
    ...
    private static void contactSearchPhone() {
        ...
        for (int i = 0; i < contacts.size(); i++) {
            if (contacts.get(i).getPhone().equals(phoneSearch)) {
                phoneIndex = i;
            }
        }
        if (phoneIndex >= 0) {
            System.out.println("#" + (phoneIndex+1) + contacts.get(phoneIndex));
        } else {
    ...
    private static void contactEdit() {
        ...
        try {
            ...
            System.out.println("Старый телефон: " + contacts.get(index-1).getPhone());
            System.out.println("Введите новый номер телефона: ");
            contacts.set(index-1, contacts.get(index-1)).setPhone(in.next(), LocalDateTime.now());
            System.out.println("Новый телефон: " + contacts.get(index-1).getPhone());
